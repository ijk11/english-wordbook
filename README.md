...
- **목록** — 648장을 처음부터 끝까지 쭉 훑어보는 화면. 스크롤하면 계속 이어 붙는다.
  - 영어·한글·예문 통합 검색 + 덱 / 종류 / 즐겨찾기 / 안 본 카드 / 어려운 카드 / 복습 예정 필터
  - 정렬: 노트 순서 · 알파벳순 · 최근 공부한 순 · 오래된 순 · 덱별
    (날짜순·덱별은 구분 머리글이 붙는다)
  - **빽빽하게** — 종이 단어장처럼 `영어 | 뜻` 두 칸으로 줄줄이
  - **가리기** — 뜻을 덮어두고 한 줄씩 탭해서 확인. 한 번 더 누르면 예문까지 펼쳐진다
  - 줄을 펼치면 예문 · 보충설명 · 공부한 날짜 · 다음 복습일이 보이고, 발음/즐겨찾기를 바로 누를 수 있다
